# Applicable Standards for Phase 4
...
- **Shopify Data Accuracy:** Hard limits exact (30 char title, 62 char subtitle, 100 char intro, 20 search terms), benchmarks use ranges

## From Design Principles

- **Progressive Disclosure:** Step 0 detects state and routes to the recommended mode. Each mode reveals only what's relevant to that listing element.
- **Solo Developer Empathy:** ASO can feel overwhelming with many elements to optimize. Break it into discrete modes with clear outputs. Celebrate progress at each step.
- **File Safety:** Never overwrite populated listing sections without confirmation. Check for existing content before writing.
- **Opinionated Defaults:** Recommend PAS framework for above-fold description. Recommend keyword-first title strategy. Provide specific options, not open-ended advice.
- **Compound Knowledge:** Each mode builds on prior modes. Keyword research informs title options. Title keywords inform subtitle (secondary keywords). All elements feed into description copy.
...
- Per-app files: `marketing/apps/{slug}/listing.md`, `marketing/apps/{slug}/keywords.md`
